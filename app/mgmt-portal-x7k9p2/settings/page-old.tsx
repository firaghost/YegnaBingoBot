      ...
      <div className="container mx-auto px-6 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-xl p-6 border border-white/20">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">Daily Withdrawal Limit (ETB)</label>
                  <input
                    type="number"
                    step="0.01"
                    min="0"
                    ...
                    onChange={(e) => {
                      ...
                      if (value === '') {
                        setSettings({...settings, dailyWithdrawalLimit: ''})
                      } else {
                        setSettings({...settings, dailyWithdrawalLimit: parseFloat(value) || 0})
                      }
                    }}
                    className="w-full px-4 py-2 bg-white/5 border border-white/20 rounded-lg text-white focus:outline-none focus:border-blue-500"
                  />
                  <p className="text-xs text-gray-400 mt-1">Total withdrawals (pending + approved) allowed per day</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">Weekly Withdrawal Limit (ETB)</label>
                  <input
                    type="number"
                    step="0.01"
                    min="0"
                    ...
                    onChange={(e) => {
                      const value = e.target.value
                      if (value === '') {
                        setSettings({...settings, weeklyWithdrawalLimit: ''})
                      } else {
                        setSettings({...settings, weeklyWithdrawalLimit: parseFloat(value) || 0})
                      }
                    }}
                    className="w-full px-4 py-2 bg-white/5 border border-white/20 rounded-lg text-white focus:outline-none focus:border-blue-500"
                  />
                  <p className="text-xs text-gray-400 mt-1">Total withdrawals (pending + approved) allowed per week</p>
                </div>
              </div>
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-xl p-6 border border-white/20">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Minimum Required Deposit (ETB)</label>
                <input
                  type="number"
                  step="0.01"
                  min="0"
                  ...
                  onChange={(e) => {
                    const value = e.target.value
                    if (value === '') {
                      setSettings({...settings, minRequiredDeposit: ''})
                    } else {
                      setSettings({...settings, minRequiredDeposit: parseFloat(value) || 0})
                    }
                  }}
                  className="w-full px-4 py-2 bg-white/5 border border-white/20 rounded-lg text-white focus:outline-none focus:border-blue-500"
                />
                <p className="text-xs text-gray-400 mt-1">User must deposit at least this much before any withdrawal unlocks</p>
              </div>
