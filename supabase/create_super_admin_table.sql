-- ============================================
-- SUPER ADMIN TABLE
-- Separate from regular admin_users
-- ============================================
...
-- Create super_admin_sessions table for session management
...
-- Create super_admin_activity_log for audit trail
...
CREATE POLICY "Service role only" ON public.super_admins
  FOR ALL 
  USING (auth.role() = 'service_role');

CREATE POLICY "Service role only" ON public.super_admin_sessions
  FOR ALL 
  USING (auth.role() = 'service_role');

CREATE POLICY "Service role only" ON public.super_admin_activity_log
  FOR ALL 
  USING (auth.role() = 'service_role');
...
BEGIN
  -- Hash the password (bf uses default 8 rounds, or use 'bf', 8 for explicit)
  v_password_hash := crypt(p_password, gen_salt('bf'));
  
  -- Insert or update
  INSERT INTO public.super_admins (username, password_hash, email, full_name)
  VALUES (p_username, v_password_hash, p_email, p_full_name)
...
-- Insert default super admin (change this password immediately!)
SELECT upsert_super_admin(
  'superadmin',
  'SuperAdmin@2025!',
  'admin@yegnabingo.com',
  'System Owner'
);
