-- ============================================
-- SUPER ADMIN TABLE (Simple Version)
-- Uses SHA256 instead of bcrypt if pgcrypto not available
-- ============================================
...
-- Function to hash password (uses bcrypt via crypt)
...
AS $$
BEGIN
  -- Use crypt with bf (blowfish/bcrypt)
  RETURN crypt(p_password, gen_salt('bf'));
END;
$$;

-- Function to verify password
CREATE OR REPLACE FUNCTION public.verify_password(p_password TEXT, p_stored_hash TEXT)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  -- Verify password
  IF verify_password(p_password, v_admin.password_hash) THEN
...
BEGIN
  -- Generate secure session token using UUIDs
...
SELECT upsert_super_admin('superadmin', 'SuperAdmin@2025!', 'admin@yegnabingo.com', 'System Owner');
